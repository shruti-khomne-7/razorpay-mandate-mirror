...
[ Principal ]
      ...
      ▼
[ Mandate Issuance ] ───(HMAC-SHA256 Signed Mandate)───┐
                                                       │
                                                       ▼
┌─────────────────┐       HTTP POST        ┌───────────────────────────────────┐
│   Buyer Agent   │ ─────────────────────► │        Mandate Mirror Core        │
│   (LLM Shopper) │   /api/v1/authorize    │                                   │
└─────────────────┘                        │  1. Gate 1: Deterministic Bounds  │
                                           │     • HMAC-SHA256 signature check │
                                           │     • Per-txn cap & allowed scope │
                                           │     • Nonce replay & expiry       │
                                           │     • Fast-fail HARD-BLOCK        │
                                           │                 │                 │
                                           │                 ▼                 │
                                           │  2. Gate 2: Forensic Investigator │
                                           │     (Gemini Tool-Calling Loop)    │
                                           │     • get_state_snapshot          │
                                           │     • get_session_history         │
                                           │     • check_category_conformance  │
                                           │     • compute_anomaly_score       │
                                           │     • recommend_outcome (advisory)│
                                           │                 │                 │
                                           │                 ▼                 │
                                           │  3. Gate 3: Guard Rechecker       │
                                           │     • Zero-drift invariant check  │
                                           │     • Overrides rogue CLEAR       │
                                           │                 │                 │
                                           │                 ▼                 │
                                           │  4. Atomic State Machine Commit   │
                                           │     • Per-bucket async mutex      │
                                           │     • CAS cumulative cap check    │
